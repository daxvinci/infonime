      ...
      const animeFetch = async()=>{
        try{
          ...
          const response = await axios.get<DetailsData>( base_url + '/anime/' + id,
            ...
            //Fetching links
          const linkResponse = await axios.get<Ogdata>( base_url + '/anime/' + id + '/streaming-links',
            {
              headers:{
                'Content-Type':'application/vnd.api+json'
              }
          ...
          //Fetching genre
          const genreResponse = await axios.get<Ogdata>( base_url + '/anime/' + id + '/genres',
            {
              headers:{
                'Content-Type':'application/vnd.api+json'
              }
                            ...
                            variants={detailsVariants}>
                                ...
                                {linkData ? (
                                      linkData.length === 0 ? (
                                        ...
                                        linkData.map((link) => 
                                        <Link className='text-blue-500' key={link.id} to={`${link?.attributes?.url}`}>
                                          {link?.attributes?.url}
                                        </Link> 
                                        )
                                      )
