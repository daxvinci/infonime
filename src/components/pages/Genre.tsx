...
const Genre = () => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      setLoading(true)
      try {
        const countResponse = await axios.get<Ogdata>('https://kitsu.io/api/edge/genres', {
          params: {
            'page[limit]': 1,
          },
        });
        ...
        const response = await axios.get<Ogdata>('https://kitsu.io/api/edge/genres',{
          params:{
        ...
        const genresWithAnimeImage = await Promise.all(
          genresData.map(async (genre) => {
            try {
              const animeResponse = await axios.get<Ogdata>('https://kitsu.io/api/edge/anime', {
                params: {
      ...
      <h1 className="text-4xl font-bold text-center mb-8">Genres</h1>
          ...
          {error ? (
                        ...
                        <div className="content grid sm:grid-cols-2 lg:grid-cols-5 grid-cols-1 gap-4">
                             {genres.map((genre) => (
                              <Link key={genre.id} to={`/genreAnimes/${genre.attributes.slug}`}>
                                <motion.div
                                  key={genre.id}
